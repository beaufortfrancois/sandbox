  ...
  class PlaybulbCandle {
    ...
    requestDevice() {
      ...
      .then(server => {
        this.server = server;
        return Promise.all([
          server.getPrimaryService(CANDLE_SERVICE_UUID).then(service => { this._services.set('candle',service); }),
          server.getPrimaryService(BATTERY_SERVICE_UUID).then(service => { this._services.set('battery', service); }),
          server.getPrimaryService(DEVICEINFO_SERVICE_UUID).then(service => { this._services.set('deviceInfo', service); }),
        ]);
      })
